 ...
 */
export function steamFPSExists(path: string): Promise<boolean> {
 ...
 * Get cached games.
 */
export function getSteamFPSGames(path: string): Promise<GameResponse[]> {
 ...
 * Cache games.
 */
export function cacheSteamFPSGames(path: string): Promise<GameResponse[]> {

    return new Promise((resolve: any, reject: any) => {

        const URL: string = `https://store.steampowered.com/search/?tags=1663&category1=998`;
