import { PriceInfoResponse, PricingsEnum } from "../../../../../client/client-server-common/common";
import axios, { AxiosResponse } from "axios";
import * as cheerio from "cheerio";
const STEAM_RATE_LIMIT_MS: number = 1000;

export function getSteamPricings(steam_games_sys_key_id: number, steam_link: string): Promise<PriceInfoResponse[]> {
            ...
            .then((response: AxiosResponse) => {
                ...
                $(".game_area_purchase_game").each((i: number, element: CheerioElement) => {
                    ...
                    if ((rawPrice.includes(`Free`) && !price) || (!isNaN(Number.parseFloat(rawPrice)) && !isNaN(price))) {
                        const pricing: PriceInfoResponse = { pricingEnum: pricingEnum, steamGamesSysKeyId: steam_games_sys_key_id, title: title, price: price, coming_soon: coming_soon, preorder: preorder, discount_percent: discountPercent, expires_dt: datePlus7Days };
