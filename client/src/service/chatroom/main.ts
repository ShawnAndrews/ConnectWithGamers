 ...
 */
export function httpUploadEmote (imageBase64: string, fileExtension: string, emotePrefix: string, emoteSuffix: string): Promise<ChatroomUploadImageResponse> {
    return new Promise((resolve: any, reject: any) => {
        axios.post(`/chatroom/emote/upload`, { emoteBase64: imageBase64, fileExtension: fileExtension, emotePrefix: emotePrefix, emoteSuffix: emoteSuffix })
        .then((result) => {
            if (result.data.error) {
                return reject(result.data.error);
            } else {
