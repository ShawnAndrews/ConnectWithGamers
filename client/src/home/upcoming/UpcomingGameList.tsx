...
const UpcomingGameList: React.SFC<IUpcomingGameListProps> = (props: IUpcomingGameListProps) => {
    ...
    return (
        <div className="upcoming-table col-6 col-xs-12 col-sm-6 py-0">
            <div className="upcoming-table-header position-relative" onClick={() => { props.goToRedirectCallback(`/games/search/filter/${GamesPresets.upcoming}`); }}>
            ...
            .map((x: GameResponse) => {
                return (
                    <div key={x.id} className="upcoming-table-container row mt-2 mx-0" onClick={() => { props.onClickGame(x.id); }}>
