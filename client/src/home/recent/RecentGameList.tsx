...
const RecentGameList: React.SFC<IRecentGameListProps> = (props: IRecentGameListProps) => {
    ...
    return (
        <div className="recently-released-table col-6 col-xs-12 col-sm-6 py-0">
            <div className="recently-released-table-header position-relative" onClick={() => { props.goToRedirectCallback(`/games/search/filter/${GamesPresets.recentlyReleased}`); }}>
            ...
            .map((x: GameResponse) => {
                return (
                    <div key={x.id} className="recently-released-table-container row mt-2 mx-0" onClick={() => { props.onClickGame(x.id); }}>
